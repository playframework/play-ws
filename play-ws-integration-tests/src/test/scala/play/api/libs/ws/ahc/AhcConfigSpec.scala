...
class AhcConfigSpec extends Specification with Mockito {
  ...
  "AhcConfigSpec" should {
    ...
    def parseThis(input: String)(implicit environment: play.api.Environment) = {
      ...
      val parser = new AhcWSClientConfigParser(defaultWsConfig, config, environment)
      parser.parse()
    }
    ...
    "case class defaults must match reference.conf defaults" in new WithEnvironment {
      val s1 = parseThis("")
    ...
    "parse ws ahc section" in new WithEnvironment {
    ...
    "with keepAlive" should {
      "parse keepAlive default as true" in new WithEnvironment {
        val actual = parseThis("""""".stripMargin)

        actual.keepAlive must beTrue
      }

      "throw exception on play.ws.ning.allowPoolingConnection" in new WithEnvironment {
        {
          parseThis("""
      ...
      "throw exception on play.ws.ning.allowSslConnectionPool" in new WithEnvironment {
        {
          parseThis("""
    ...
    "with SSL options" should {
      ...
      "with context" should {
        ...
        }.pendingUntilFixed("Uses substitutelogger now, so test fails")
