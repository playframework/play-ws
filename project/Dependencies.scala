...
object Dependencies {
  ...
  val logback = Seq("ch.qos.logback" % "logback-core" % "1.5.29")
  ...
  val playJson = Seq("org.playframework" %% "play-json" % "3.1.0-M9")

  val slf4jApi = Seq("org.slf4j" % "slf4j-api" % "2.0.17")

  val jakartaInject = Seq("jakarta.inject" % "jakarta.inject-api" % "2.0.1")

  val sslConfigCore = Seq("com.typesafe" %% "ssl-config-core" % "0.7.1")
  ...
  val nettyVersion    = "4.1.131.Final" // Keep in sync with the netty version netty-reactive-streams uses (see below)
  val asyncHttpClient = Seq(
    ("org.asynchttpclient" % "async-http-client" % "3.0.7") // 2.12.x comes with outdated netty-reactive-streams and netty, so we ...
