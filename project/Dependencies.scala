...
object Dependencies {
  ...
  val logback = Seq("ch.qos.logback" % "logback-core" % "1.5.34")
  ...
  val mockito = Seq("org.mockito" % "mockito-core" % "5.23.0")
  ...
  val nettyVersion    = "4.1.135.Final" // Keep in sync with the netty version netty-reactive-streams uses (see below)
  val asyncHttpClient = Seq(
    ("org.asynchttpclient" % "async-http-client" % "3.0.11") // 2.12.x comes with outdated netty-reactive-streams and netty, so we ...
